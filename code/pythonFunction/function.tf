...
resource "azurerm_resource_group_template_deployment" "function" {
  ...
  parameters_content = jsonencode({
    ...
    subnetId = {
      value = jsondecode(azurerm_resource_group_template_deployment.subnet_function.output_content).subnetId.value
    },
  ...
  template_content = file("${path.module}/arm/function.json")
}
...
data "azurerm_monitor_diagnostic_categories" "diagnostic_categories_function" {
  resource_id = jsondecode(azurerm_resource_group_template_deployment.function.output_content).functionId.value  # azapi_resource.function.id
}

resource "azurerm_monitor_diagnostic_setting" "diagnostic_setting_function" {
  name                       = "logAnalytics"
  target_resource_id         = jsondecode(azurerm_resource_group_template_deployment.function.output_content).functionId.value  # azapi_resource.function.id
